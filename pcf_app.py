...
class PcfApp:
    ...
    def _process_files(self, folder, pcf_files):
        ...
        # 解析所有 PCF 文件
        parser = PcfParser(self.data_collector)
        for i, pcf_file in enumerate(pcf_files):
            try:
                parser.process_single_file(pcf_file, i)
    ...
    def _add_points_to_tree(self, table_name, display_name, col_x, col_y, col_z, params):
        ...
        try:
            columns, rows = self.db_helper.fill_data_table(
                f"SELECT {col_x}, {col_y}, {col_z}, NPD, TYPE FROM {table_name} WHERE UCI=? AND FILENAME=?",
                params)
        except Exception:
            return
