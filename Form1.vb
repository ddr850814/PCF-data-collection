...
Public Class Form1
    ...
    Private Sub WritePreviously(report As Boolean, filename As Integer, ByRef Pipeline As Boolean, rowP As List(Of String), ByRef row As List(Of String), Title As String, ByRef ENDPOINT As List(Of String))
        ...
        ElseIf row.Any(Function(n) n <> "") Then
            If report = True Then
                If Title = "SUPPORT" Then
                    InsertSQLiteTableSupport(row, filename)
                ElseIf Title = "MESSAGE" Then
                    row(2) = "MS" & MESSAGE
                    MESSAGE += 1
                    InsertSQLiteTableMessage(row, filename)
                ElseIf Title = "BOLT" Then
                    InsertSQLiteTableBolt(row, filename)
                ElseIf Title = "PIPE" Or Title = "PIPE_FIXED" Or Title = "BEND" Then
                    InsertSQLiteTablePipe(Title, row, filename)
                ElseIf Title = "TEE_STUB" Then
                    row(2) = "TS" & TEESTUB
                    TEESTUB += 1
    ...
    Private Sub BgWorker_DoWork(sender As Object, e As DoWorkEventArgs) Handles BgWorker.DoWork
        ...
        For Each Type In Types
            ...
            ElseIf Type = "PIPE" Or Type = "PIPE_FIXED" Or Type = "BEND" Then
            ...
            Using reader As New StreamReader(divisibleByThree(i), Encoding.GetEncoding("GBK"))
                ...
                While line IsNot Nothing
                    ...
                    ElseIf Types.Contains(line.Split(" "c)(0).Replace("-", "_")) Then
                        WritePreviously(report, i, Pipeline, rowP, row, Title, ENDPOINT)
            ...
            If result = DialogResult.OK Then
                ...
                For Each Type In Types
                    ...
                    If oDataTable.Columns.Count > 0 And oDataTable.Columns.Contains("FILENAME") Then
                        ...
                        Next
                        For j = 0 To oDataTable.Columns.Count - 1
                            tb.GetRow(0).GetCell(j).SetCellValue(oDataTable.Columns.Item(j).Caption)
                        Next
                        tb.CreateFreezePane(0, 1)
                        For i = 0 To oDataTable.Rows.Count - 1
                            For j = 0 To oDataTable.Columns.Count - 1
                                tb.GetRow(i + 1).GetCell(j).SetCellValue(oDataTable.Rows(i).Item(j).ToString())
                            Next
                        Next
                    End If
                Next
