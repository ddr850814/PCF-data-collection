...
Public Class PcfDataCollector
    ...
    Public ReadOnly Property InstrumentReturnData As List(Of String())
    Public ReadOnly Property TrapOffsetData As List(Of String())
    ...
    Public ReadOnly Property AttributeData As List(Of String())
    ...
    Public Sub New()
        ...
        InstrumentReturnData = New List(Of String())
        TrapOffsetData = New List(Of String())
        ...
        AttributeData = New List(Of String())
    End Sub
    ...
    Public Sub Reset()
        ...
        BoltData.Clear()
        MessageData.Clear()
        InstrumentReturnData.Clear()
        TrapOffsetData.Clear()
    ...
    ''' <summary>
    ''' 添加 InstrumentReturn 数据
    ''' </summary>
    Public Sub AddInstrumentReturn(data As String())
        InstrumentReturnData.Add(data)
    End Sub

    ''' <summary>
    ''' 添加 TrapOffset 数据
    ''' </summary>
    Public Sub AddTrapOffset(data As String())
        TrapOffsetData.Add(data)
    End Sub
    ...
    ''' 添加组件属性数据
    ''' </summary>
    Public Sub AddAttribute(filename As String, uci As String, attrName As String, attrValue As String)
        Dim data As String() = {filename, uci, attrName, attrValue}
    ...
    Public Sub CreateAllTables()
        ...
        CreateBoltTable()
        CreateMessageTable()
        CreateInstrumentReturnTable()
        CreateTrapOffsetTable()
    ...
    Private Sub CreateInstrumentReturnTable()
        DbHelper.DropTableIfExists("INSTRUMENT_RETURN")
        DbHelper.CreateTable("CREATE TABLE IF NOT EXISTS INSTRUMENT_RETURN (FILENAME INTEGER,ITEMCODE TEXT,TAG TEXT,UCI TEXT,ITEMDESCRIPTION TEXT,SKEY TEXT,COMPONENTIDENTIFIER INTEGER,MASTERCOMPONENTIDENTIFIER INTEGER)")
    End Sub

    Private Sub CreateTrapOffsetTable()
        DbHelper.DropTableIfExists("TRAP_OFFSET")
        DbHelper.CreateTable("CREATE TABLE IF NOT EXISTS TRAP_OFFSET (FILENAME INTEGER,ITEMCODE TEXT,TAG TEXT,UCI TEXT,ITEMDESCRIPTION TEXT,SKEY TEXT,COMPONENTIDENTIFIER INTEGER,MASTERCOMPONENTIDENTIFIER INTEGER)")
    End Sub

    Private Sub CreateAttributeTable()
        DbHelper.DropTableIfExists("COMPONENT_ATTRIBUTES")
        DbHelper.CreateTable("CREATE TABLE IF NOT EXISTS COMPONENT_ATTRIBUTES (FILENAME INTEGER,UCI TEXT,ATTRIBUTE_NAME TEXT,ATTRIBUTE_VALUE TEXT,PRIMARY KEY (FILENAME, UCI, ATTRIBUTE_NAME))")
        DbHelper.ExecuteNonQuery("CREATE INDEX IF NOT EXISTS idx_attr_uci ON COMPONENT_ATTRIBUTES(UCI)")
        DbHelper.ExecuteNonQuery("CREATE INDEX IF NOT EXISTS idx_attr_name ON COMPONENT_ATTRIBUTES(ATTRIBUTE_NAME)")
    End Sub
    ...
    Public Sub InsertAllData(Optional progressCallback As Action(Of String, Integer, Integer) = Nothing)
        ...
        ' 插入 MESSAGE 数据
        progressCallback?.Invoke("正在插入 MESSAGE 数据...", 6, 10)
        BulkInsertSimple(
            "INSERT INTO MESSAGE (FILENAME,ITEMCODE,TEXT,UCI) VALUES (@p0,@p1,@p2,@p3)",
            MessageData, 4, progressCallback)

        ' 插入 INSTRUMENT_RETURN 数据
        progressCallback?.Invoke("正在插入 INSTRUMENT_RETURN 数据...", 7, 10)
        BulkInsertSimple(
            "INSERT INTO INSTRUMENT_RETURN (FILENAME,ITEMCODE,TAG,UCI,ITEMDESCRIPTION,SKEY,COMPONENTIDENTIFIER,MASTERCOMPONENTIDENTIFIER) VALUES (@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7)",
            InstrumentReturnData, 8, progressCallback)

        ' 插入 TRAP_OFFSET 数据
        progressCallback?.Invoke("正在插入 TRAP_OFFSET 数据...", 8, 10)
        BulkInsertSimple(
            "INSERT INTO TRAP_OFFSET (FILENAME,ITEMCODE,TAG,UCI,ITEMDESCRIPTION,SKEY,COMPONENTIDENTIFIER,MASTERCOMPONENTIDENTIFIER) VALUES (@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7)",
            TrapOffsetData, 8, progressCallback)

        ' 插入 ATTRIBUTE 数据
        progressCallback?.Invoke("正在插入 ATTRIBUTE 数据...", 9, 10)
        BulkInsertSimple(
            "INSERT OR REPLACE INTO COMPONENT_ATTRIBUTES (FILENAME,UCI,ATTRIBUTE_NAME,ATTRIBUTE_VALUE) VALUES (@p0,@p1,@p2,@p3)",
            AttributeData, 4, progressCallback)

        progressCallback?.Invoke("数据插入完成", 10, 10)
    End Sub
    ...
    Public Sub CreateTotalView()
        ...
        For i = 0 To ComponentTypesField.Count - 1
            ...
            ElseIf type = "INSTRUMENT_RETURN" Then
                part &= $" UNION ALL SELECT FILENAME,COALESCE(ITEMCODE,'INSTRUMENT_RETURN') AS ITEMCODE,UCI,ITEMDESCRIPTION,COMPONENTIDENTIFIER FROM INSTRUMENT_RETURN"
            ElseIf type = "TRAP_OFFSET" Then
                part &= $" UNION ALL SELECT FILENAME,COALESCE(ITEMCODE,'TRAP_OFFSET') AS ITEMCODE,UCI,ITEMDESCRIPTION,COMPONENTIDENTIFIER FROM TRAP_OFFSET"
            Else
    ...
    ''' 创建带属性的组件视图
    ''' </summary>
    Public Sub CreateComponentWithAttributesView()
        ' 组件属性视图
        Dim sql = "CREATE VIEW IF NOT EXISTS COMPONENTS_WITH_ATTR AS
        SELECT 
            c.FILENAME,
            c.UCI,
            c.ITEMCODE,
        ...
        ' PIPELINE 属性视图
        Dim pipelineSql = "CREATE VIEW IF NOT EXISTS PIPELINE_WITH_ATTR AS
        SELECT 
            p.FILENAME,
            p.REFERENCE,
            p.PIPINGSPEC,
        ...
        DbHelper.ExecuteNonQuery(pipelineSql)
    End Sub
...
#Region "兼容层 - 供 Form1 直接调用的方法"
    ...
    ''' <summary>
    ''' 插入 InstrumentReturn 数据（兼容方法）
    ''' </summary>
    Public Sub InsertSQLiteTableInstrumentReturn(row As List(Of String), filename As Integer)
        ' INSTRUMENT_RETURN: FILENAME, ITEMCODE, TAG, UCI, ITEMDESCRIPTION, SKEY, COMPONENTIDENTIFIER, MASTERCOMPONENTIDENTIFIER
        ...
        AddInstrumentReturn(data)
    End Sub

    ''' <summary>
    ''' 插入 TrapOffset 数据（兼容方法）
    ''' </summary>
    Public Sub InsertSQLiteTableTrapOffset(row As List(Of String), filename As Integer)
        ' TRAP_OFFSET: FILENAME, ITEMCODE, TAG, UCI, ITEMDESCRIPTION, SKEY, COMPONENTIDENTIFIER, MASTERCOMPONENTIDENTIFIER
        Dim data As String() = {filename.ToString(), ValueS(row(0)), ValueS(row(1)), ValueS(row(2)), ValueS(row(3)), ValueS(row(4)), ValueI(row(5)), ValueI(row(6))}
        AddTrapOffset(data)
    End Sub
